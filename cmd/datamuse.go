...
import (
	...
	"github.com/pierow2k/polyhymnia/internal/displayresults"
...
var (
	...
	displayOptions displayresults.DisplayOptions
)
...
func setDisplayOptionsFromMetadata(metadata string, opts *displayresults.DisplayOptions) {
...
func runDatamuseQuery(cmd *cobra.Command, args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("you must provide a search term as the first argument. Use --help for more information.")
	}

	// Assign the first argument as SearchTerm
	queryParams.SearchTerm = args[0]
	if !queryParams.Ml && !queryParams.Sl && !queryParams.Sp && len(queryParams.RelCode) == 0 {
		fmt.Println("At least one action flag (--means-like, --related-word, --sounds-like, or --spelled-like) must be specified")
		return fmt.Errorf("no action flag provided")
	}
	...
	results, err := datamuseapi.QueryAPI(queryParams)
	if err != nil {
		return fmt.Errorf("error querying Datamuse API: %v", err)
	}

	// Display results using the displayresults package.
	displayresults.DisplayResults(results, displayOptions)
